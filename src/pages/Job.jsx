...
import TimeAgo from '../component/UI/TimeAgo';
...
const Job = () => {
  ...
  if (isSuccess) {
    ...
    header = job.map((job) => (
      <div className="relative h-[40vh] mb-8" key={job.id}>
        <img
          src={job.logoUrl}
          alt="Banner Image"
    ...
    content = job.map((job) => (
      ...
      >
        <div className="w-full md:mr-4">
          ...
          <h1 className="text-4xl font-bold mb-2 capitalize">{job.role}</h1>

          <p className="flex items-center gap-2 text-primary text-lg font-semibold capitalize mb-2">
            <TimeAgo date={job.postedAt} /> •{' '}
          ...
          <div className="my-12">
            <div className="flex sm:mt-0 items-center">
              <span className="text-lg font-bold mr-2">Apply here: </span>
              {job.contact}
            </div>
          </div>
        </div>
